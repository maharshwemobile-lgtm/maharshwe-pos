...
function SettingsPage({ api, toast }) {
  ...
  async function downloadBackup(){ const res=await fetch(apiUrl('/api/backup'),{headers:{Authorization:`Bearer ${safeStorage.getItem('ms_token')||''}`}}); const data=await res.text(); if(!res.ok){ toast(data.slice(0,180)||'Backup download failed','error'); return; } const blob=new Blob([data],{type:'application/json;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='maharshwe-pos-full-backup-'+today()+'.json'; a.click(); URL.revokeObjectURL(a.href); toast('Full backup downloaded'); api.get('/api/backup/status').then(x=>!x.error&&setBackupStatus(x)); }
  async function restoreBackup(e){ const file=e.target.files?.[0]; if(!file) return; try { const json=JSON.parse(await file.text()); const res=await api.post('/api/restore', json); if(res.error) toast(res.error,'error'); else { toast('Full backup restored'); load(); } } catch(err) { toast('Invalid backup JSON file','error'); } e.target.value=''; }
  ...
  return <div style={{ display:'grid', gridTemplateColumns:isMobileSettings?'minmax(0,1fr)':'240px minmax(0,1fr)', gap:16, minWidth:0 }}>
    ...
    <div style={{ display:'grid', gap:16, minWidth:0 }}>
      ...
      {section==='backup' && <SettingsPanel>
        <h2 style={cardTitle}>Full Backup / Restore</h2>
        <p style={{ color:'#777', lineHeight:1.7 }}>Download Full Backup creates one complete restore-ready JSON file with products, sales, repairs, accounting ledger, settings, users, and logs. Keep it private because it includes settings and password hashes.</p>
        {backupStatus&&<div style={{ background:backupStatus.downloadedToday?'#EAF3DE':'#FFF4DA', color:backupStatus.downloadedToday?'#3B6D11':'#854F0B', padding:12, borderRadius:8, fontSize:14, marginBottom:12, lineHeight:1.7 }}><b>{backupStatus.downloadedToday?'Full Backup Downloaded Today':'Full Backup is due today'}</b><br/>Date: {backupStatus.today}<br/>Format: {backupStatus.format || 'full'}<br/>Auto Backup: {backupStatus.serverBackupExists?'Ready':'Creating'}<br/>Last Download: {backupStatus.lastDownloadedDate || '-'}<br/>Products: {backupStatus.counts?.products ?? 0} | Sales: {backupStatus.counts?.sales ?? 0} | Repairs: {backupStatus.counts?.repairs ?? 0} | Ledger: {backupStatus.counts?.ledger ?? 0}</div>}
        <button style={S.btn('primary')} onClick={downloadBackup}>Download Full Backup</button> <button style={S.btn()} onClick={()=>backupRef.current?.click()}>Restore Full Backup JSON</button>
