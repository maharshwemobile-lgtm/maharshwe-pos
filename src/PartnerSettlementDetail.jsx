...
export default function PartnerSettlementDetail({ data, onClose, onRefresh, onError }) {
  ...
  return <div className="psw-drawer-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <aside className="psw-drawer">
      ...
      <section className="psw-detail-section"><h4>Included Repair Jobs</h4>{ledger.map((x) => <div className="psw-detail-row" key={x.id}><div><b>{x.partnerRepairNumber} → {x.providerRepairNumber}</b><span>{x.customerPaid ? 'Customer Paid' : 'Customer Not Paid'} · {dateText(x.completedAt)}</span></div><div><b>{money(x.providerDue)}</b><small>Profit {money(x.partnerProfit)}</small></div></div>)}</section>
