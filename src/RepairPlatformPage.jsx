...
function IncomingQueueTab() {
  const [data, setData] = useState({ incoming: [], total: 0, totalPages: 1 });
  ...
  const handleSyncLedger = async () => {
    setSyncing(true);
    try {
      const result = await apiFetch('/api/repair-platform/ledger/sync', { method: 'POST' });
      notify('success', result?.message || 'Ledger sync complete');
      load();
    } catch (err) {
      notify('error', err?.message || 'Ledger sync failed');
    } finally {
      setSyncing(false);
    }
  };
  ...
  return (
    <div className="repair-incoming-queue">
      {toast && (
        <div className={`rp-toast rp-toast-${toast.type}`}>{toast.text}</div>
      )}
      <div className="rp-toolbar">
        ...
        >
          ...
          {Object.entries(REFERRAL_STATUS_LABELS).map(([v, l]) => (
        ...
        <button
          ...
          disabled={syncing}
        >
          {syncing ? <Loader2 size={14} className="spin" /> : <PackageCheck size={14} />}
          Sync Ledger (Phase 9)
        </button>
      </div>
      {loading ? (
        <div className="rp-loading"><Loader2 size={24} className="spin" /> Loading incoming repairs…</div>
      ) : data.incoming.length === 0 ? (
        <div className="rp-empty">No incoming repairs from partner shops.</div>
      ) : (
        <>
              ...
              <tbody>
                {data.incoming.map((row) => (
                  <tr key={row.referralId}>
                    ...
                    <td>{row.sourceShopName}</td>
                    <td>{row.customerName}{row.customerPhone ? <><br /><small>{row.customerPhone}</small></> : null}</td>
                    <td>{[row.deviceBrand, row.deviceModel].filter(Boolean).join(' ')}</td>
                    <td className="rp-cell-problem">{row.problem}</td>
                    <td><span className={`rp-status-badge rp-status-${row.referralStatus?.toLowerCase()}`}>{REFERRAL_STATUS_LABELS[row.referralStatus] ?? row.referralStatus}</span></td>
                    <td>{row.providerRepairNumber || '—'}</td>
                    <td>{row.providerRepairStatus || '—'}</td>
          ...
          <div className="rp-pagination">
            <button className="rp-btn rp-btn-icon" onClick={() => setPage((p) => p - 1)} disabled={page <= 1}><ChevronLeft size={16} /></button>
            <span>Page {data.page ?? page} / {data.totalPages}</span>
            <button className="rp-btn rp-btn-icon" onClick={() => setPage((p) => p + 1)} disabled={page >= data.totalPages}><ChevronRight size={16} /></button>
          </div>
        </>
      )}
    </div>
...
export default function RepairPlatformPage() {
  ...
  const isProviderAdmin = session?.user?.role === 'SUPER_ADMIN' || session?.user?.role === 'SHOP_ADMIN';
  ...
  return (
    <section className="repair-platform-page">
      ...
      <div className="repair-tab-bar">
        <button type="button" className={activeTab === 'jobs' ? 'active' : ''} onClick={() => setActiveTab('jobs')}>
          <Wrench size={15} /> My Repairs
        </button>
        {isProviderAdmin && (
          <button type="button" className={activeTab === 'incoming' ? 'active' : ''} onClick={() => setActiveTab('incoming')}>
            <Inbox size={15} /> Incoming Queue
          </button>
        )}
      </div>

      {activeTab === 'incoming' && isProviderAdmin ? (
        <IncomingQueueTab />
      ) : (
        <>
