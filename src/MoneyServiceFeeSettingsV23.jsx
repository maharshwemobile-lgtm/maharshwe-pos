...
export default function MoneyServiceFeeSettingsV23() {
  ...
  const save = async (event) => {
    ...
    try {
      ...
      setMessage('Fee settings saved'); await load();
  ...
  return <section className="finance-catalog-section open">
    ...
    <div className="finance-catalog-section-body">
      ...
      <form className="finance-fee-settings" onSubmit={save}>
        ...
        <div className="finance-fee-list">{methods.map((row) => <article key={row.id}><div><b>{row.name}</b><small>{row.code}</small></div><label><span>Transfer %</span><input type="number" min="0" max="100" step="0.01" value={draft[`${row.code}_TRANSFER`] ?? 0} onChange={(event) => change(`${row.code}_TRANSFER`, event.target.value)}/></label><label><span>Cash Out %</span><input type="number" min="0" max="100" step="0.01" value={draft[`${row.code}_CASH_OUT`] ?? 0} onChange={(event) => change(`${row.code}_CASH_OUT`, event.target.value)}/></label></article>)}</div>
