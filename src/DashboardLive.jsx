...
export default function DashboardLive({ onNavigate }) {
  ...
  const metrics = [
    { icon: Wallet, label: 'ယနေ့ စုစုပေါင်းဝင်ငွေ', value: dashboard.todayTotalIncome, detail: 'Sales + Repair received + Service profit', tone: 'green' },
    ...
    { icon: Wrench, label: 'ဖုန်းပြင်ရရှိငွေ', value: dashboard.repairIncome, detail: `${Number(dashboard.repairPayments || 0)} repair payments`, tone: 'gold' },
    { icon: CircleDollarSign, label: 'ယနေ့ စုစုပေါင်းအမြတ်', value: dashboard.todayProfit, detail: 'Sales + Repair + Money service − Expense', tone: 'violet' },
  ...
  const closeBusinessDay = async () => {
    if (!window.confirm(`${businessDate} ရက်အတွက် Daily Closing လုပ်မှာ သေချာပါသလား? ပိတ်ပြီးပါက ပြန်ပိတ်၍မရပါ။`)) return;
    setClosing(true);
    setNotice('');
    setError('');
    try {
  ...
  return (
    <div className="business-control-dashboard">
      ...
      {data ? <>
        ...
        <section className="bc-main-grid">
          <article className="bc-panel bc-trend-panel">
            ...
            <div className="bc-trend-summary">
              ...
              <span>Stock Value <b>{money(dashboard.stockBalance)}</b></span>
            </div>
          </article>
        ...
        <section className="bc-secondary-grid">
          <article className="bc-panel bc-closing-panel">
            ...
            <div className="bc-closing-summary">
              ...
              <span>Total Profit <b>{money(dashboard.todayProfit)}</b></span>
            </div>
            {data.closing ? <div className="bc-closed-box"><CheckCircle2 size={28} /><div><b>{data.closing.businessDate} Closed</b><span>{data.closing.closedAt ? new Date(data.closing.closedAt).toLocaleString() : ''}</span><p>{data.closing.note || 'No closing note.'}</p></div></div> : <>
              ...
              <small className="bc-helper">Closing creates one locked PostgreSQL snapshot for this shop and date.</small>
            </>}
          </article>
