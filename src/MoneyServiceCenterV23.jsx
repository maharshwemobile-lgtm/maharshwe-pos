...
const serviceHint = (mode) => (mode === 'CASH_OUT'
  ? 'Customer wallet ထဲကငွေကို shop cash/account မှ ထုတ်ပေးတဲ့ tab ပါ။'
  : 'Customer cash လက်ခံပြီး linked wallet နဲ့ ငွေလွှဲတဲ့ Cash In tab ပါ။');
...
function Wizard({ settings, initialMode = 'TRANSFER', onSaved }) {
  ...
  return <section className="msc-wizard">
    ...
    {message ? <div className="msc-message">{message}</div> : null}
    {step === 1 ? <div className="msc-step"><h3>1. Choose Wallet & Amount</h3><p>{form.mode === 'CASH_OUT' ? 'Cash Out သီးသန့် tab ဖြစ်ပြီး customer ကို cash ထုတ်ပေးမယ့် wallet/account ကိုရွေးပါ။' : 'Cash In သီးသန့် tab ဖြစ်ပြီး cash လက်ခံပြီး ငွေလွှဲမယ့် wallet/account ကိုရွေးပါ။'}</p><div className="msc-flow-card-grid"><article><b>{serviceTitle(form.mode)}</b><small>{form.mode === 'CASH_OUT' ? 'Wallet balance +, Cash paid out' : 'Cash/account receives payment, Wallet transfers out'}</small></article><article><b>Linked Wallet</b><small>Project Settings → Wallet Links မှ Money Service On ဖြစ်သော wallet များသာပေါ်မယ်။</small></article></div><div className="msc-form-grid"><label><span>Linked Wallet / Payment Type *</span><select value={form.paymentMethodId} onChange={(e) => setForm({ ...form, paymentMethodId: e.target.value })}><option value="">Choose wallet</option>{methods.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label><label><span>{form.mode === 'CASH_OUT' ? 'Cash Pay-out Account *' : 'Cash Receive Account *'}</span><select value={form.cashAccountId} onChange={(e) => setForm({ ...form, cashAccountId: e.target.value })}><option value="">Choose account</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.name} · {money(a.balance)}</option>)}</select></label><label className="wide"><span>Amount *</span><input type="number" min="1" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="0" autoFocus/></label></div></div> : null}
