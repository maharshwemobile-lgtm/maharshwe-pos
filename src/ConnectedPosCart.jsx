  ...
  return (
    <aside className="connected-pos-cart">
      <div className="connected-pos-cart-list">
        {cart.map((line) => (
          <article key={line.key}>
            <div className="connected-pos-cart-product">
              ...
              <small>Stock {Math.max(0, Number(line.stockQuantity || 0) - Number(reservedMap.get(line.id) || 0))}</small>
            ...
            <div className="connected-pos-cart-price"><span>Price</span><input type="number" min="0" value={line.unitPrice} onChange={(event) => onPatch(line.key, { unitPrice: event.target.value })} /></div>
        ...
        {!cart.length ? <div className="connected-pos-cart-empty"><ShoppingCart size={34} /><b>Cart is empty</b></div> : null}
      </div>
      {children}
    </aside>
  );
