...
app.post('/api/reports/export-all', requireToken, (req, res) => {
  const report = {
    generatedAt: new Date().toISOString(),
    shop: req.body.shop || req.body.shopName || 'Mahar Shwe Mobile',
    summary: {
      products: (req.body.products || []).length,
      sales: (req.body.sales || []).length,
      repairs: (req.body.repairs || []).length,
      buyins: (req.body.buyins || []).length,
      expenses: (req.body.expenses || []).length,
      totalSalesAmount: (req.body.sales || []).reduce((sum, s) => sum + Number(s.payable || 0), 0),
      totalExpenseAmount: (req.body.expenses || []).filter(e => e.type === 'outcome').reduce((sum, e) => sum + Number(e.amount || 0), 0),
      totalIncomeAmount: (req.body.expenses || []).filter(e => e.type === 'income').reduce((sum, e) => sum + Number(e.amount || 0), 0),
    },
    salesByUser: req.body.salesByUser || [],
    ...
    data: {
      ...
      sales: req.body.sales || [],
      repairs: req.body.repairs || [],
      buyins: req.body.buyins || [],
      expenses: req.body.expenses || [],
      cashiers: req.body.cashiers || [],
    },
  };
