...
export default function App() {
  ...
  const [shopConfig, setShopConfig] = useState(() => JSON.parse(localStorage.getItem('ms_shop_config') || 'null') || {
    shopName: 'Mahar Shwe Mobile',
    address: 'ဆီဆိုင်မြို့',
    phone: '09778394052',
    logoUrl: 'https://raw.githubusercontent.com/maharshwemobile-lgtm/DataForPublic/refs/heads/main/LOGO%20PSD%20(1).png',
  ...
  const generateAppToken = () => {
    ...
    setShopConfig(prev => ({ ...prev, appToken: token }));
    addLog('Admin', 'Generate Token', 'New POS API token generated');
    showNotification('Token အသစ် ထုတ်ပြီးပါပြီ', 'success');
  };
  ...
  const exportAllReportsByToken = async () => {
    try {
      const res = await fetch('/api/reports/export-all', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-pos-token': shopConfig.appToken || '' },
        body: JSON.stringify(buildReportSnapshot())
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data.ok === false) throw new Error(data.message || 'Report export failed');
      const blob = new Blob([JSON.stringify(data.report, null, 2)], { type: 'application/json;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `MaharShwe_All_Reports_${new Date().toISOString().slice(0,10)}.json`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
      showNotification('Token API ဖြင့် Report အကုန် Export ထုတ်ပြီးပါပြီ', 'success');
    } catch (err) {
      showNotification(err.message || 'Report export မအောင်မြင်ပါ', 'error');
    }
  };
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5">
          <div className="text-center space-y-2">
            <img src={shopConfig.logoUrl || 'https://raw.githubusercontent.com/maharshwemobile-lgtm/DataForPublic/refs/heads/main/LOGO%20PSD%20(1).png'} className="w-16 h-16 rounded-2xl object-cover mx-auto border border-amber-500/40" onError={(e)=>{e.currentTarget.style.display='none'}} />
      ...
      <header className="bg-slate-950 border-b border-slate-800 sticky top-0 z-30 px-4 py-3 shadow-lg">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            <img src={shopConfig.logoUrl || 'https://raw.githubusercontent.com/maharshwemobile-lgtm/DataForPublic/refs/heads/main/LOGO%20PSD%20(1).png'} alt="မဟာရွှေ မိုဘိုင်း" className="w-11 h-11 rounded-xl object-cover border border-amber-500/30 shadow-md shadow-yellow-500/10" onError={(e) => { e.target.style.display = 'none'; }} />
            <div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-3">
                  ...
                  <div className="flex gap-2"><input value={shopConfig.appToken || ''} onChange={e=>setShopConfig({...shopConfig, appToken:e.target.value})} placeholder="POS API Token" className="min-w-0 flex-1 bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 font-mono" /><button onClick={generateAppToken} className="bg-amber-500 text-slate-950 font-bold px-3 rounded-lg text-xs">Token ထုတ်</button></div>
                  ...
                  <button onClick={exportAllReportsByToken} className="w-full bg-emerald-500 text-slate-950 font-bold py-2 rounded-lg text-xs">🔐 Token API Report အကုန် Export</button>
                </div>
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-3">
                  ...
                  <button onClick={() => setShopConfig({...shopConfig, logoUrl:'https://raw.githubusercontent.com/maharshwemobile-lgtm/DataForPublic/refs/heads/main/LOGO%20PSD%20(1).png'})} className="w-full bg-slate-800 border border-slate-700 text-amber-300 font-bold py-2 rounded-lg text-xs">Logo ပြန်ထည့်</button>
