...
function ledgerWhere(req){const query=String(req.query.q||'').trim();const status=String(req.query.status||'').trim();const payment=String(req.query.payment||'').trim();const soldAt=soldAtRange(req.query.from,req.query.to);return{shopId:req.auth.shopId,...(status?{status}:{}),...(soldAt?{soldAt}:{}),...(payment==='CREDIT'?{paymentStatus:'PENDING'}:payment?{payments:{some:{method:payment}}}:{}),...(query?{OR:[{invoiceNumber:{contains:query,mode:'insensitive'}},{customer:{name:{contains:query,mode:'insensitive'}}},{customer:{phone:{contains:query,mode:'insensitive'}}},{items:{some:{productNameSnapshot:{contains:query,mode:'insensitive'}}}},{items:{some:{variantNameSnapshot:{contains:query,mode:'insensitive'}}}},{items:{some:{imeiSerial:{contains:query,mode:'insensitive'}}}}]}:{})}}
...
module.exports=function attachCommerceLedgerApi(app){
  ...
  app.get('/api/sales/ledger',...access,core.route(async(req,res)=>{
    const page=Math.max(1,parseInt(req.query.page||'1',10)||1);const limit=Math.min(100,Math.max(1,parseInt(req.query.limit||'20',10)||20));const where=ledgerWhere(req);const summaryWhere={...where,status:'COMPLETED'};
    const [total,rows,summary]=await core.prisma.$transaction([core.prisma.sale.count({where}),core.prisma.sale.findMany({where,include:{customer:{select:{id:true,name:true,phone:true}},user:{select:{id:true,name:true}},payments:{where:{status:'PAID'},orderBy:{paidAt:'asc'},take:1},items:{orderBy:{createdAt:'asc'}}},orderBy:{soldAt:'desc'},skip:(page-1)*limit,take:limit}),core.prisma.sale.aggregate({where:summaryWhere,_count:{id:true},_sum:{total:true,discount:true,profitTotal:true}})]);
  ...
  app.get('/api/sales/:id',...access,core.route(async(req,res)=>{
    const sale=await core.prisma.sale.findFirst({where:identity(String(req.params.id||''),req.auth.shopId),include:{customer:{select:{id:true,name:true,phone:true,balance:true}},user:{select:{id:true,name:true}},payments:{orderBy:{paidAt:'asc'}},items:{orderBy:{createdAt:'asc'}}}});
    if(!sale)throw new core.CommerceError(404,'Sale not found');
    const result=mapSale(sale,canViewCost(req));result.voidedAt=sale.voidedAt;result.voidReason=sale.voidReason;result.payments=sale.payments.map(p=>({id:p.id,method:p.method,status:p.status,amount:core.number(p.amount),reference:p.reference,paidAt:p.paidAt}));
