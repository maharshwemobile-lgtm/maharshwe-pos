...
function wrap(handler) {
  return async (req, res) => {
    try {
      ...
      console.error('Business Control V2 API:', error);
  ...
  if (!schemaPromise) {
    schemaPromise = prisma.$transaction(async (tx) => {
      const statements = [
        ...
        `CREATE TABLE IF NOT EXISTS business_other_income (
          id UUID PRIMARY KEY,
          shop_id UUID NOT NULL REFERENCES shops(id) ON DELETE CASCADE,
        ...
        `ALTER TABLE daily_closings ADD COLUMN IF NOT EXISTS other_income_total NUMERIC(14,2) NOT NULL DEFAULT 0`,
        ...
        `ALTER TABLE daily_closings DROP CONSTRAINT IF EXISTS daily_closings_amounts_nonnegative_check`,
        `ALTER TABLE daily_closings ADD CONSTRAINT daily_closings_amounts_nonnegative_check CHECK (
          sales_total >= 0 AND service_income_total >= 0 AND cash_balance >= 0 AND
          kpay_balance >= 0 AND wave_pay_balance >= 0 AND repair_income_total >= 0 AND
          expense_total >= 0 AND other_income_total >= 0 AND receivable_total >= 0 AND payable_total >= 0
        )`,
      ];
...
async function supplierSummary(shopId, businessDate) {
  try {
    ...
    return { payable: number(rows[0]?.payable), paidToday: number(rows[0]?.paidToday) };
  } catch (error) {
...
function closingJson(row) {
  ...
  return {
    ...
    otherIncomeTotal: number(row.otherIncomeTotal),
...
async function buildOverview(shopId, businessDate) {
  ...
  const income = otherIncomeRows[0] || {};
  ...
  return {
    ...
    dashboard: {
      ...
      otherIncomeCount: Number(income.count || 0),
    ...
    recentOtherIncome: recentOtherIncome.map((row) => ({ ...row, amount: number(row.amount) })),
...
async function resolveAccount(tx, shopId, requestedAccountId, method) {
  if (requestedAccountId) {
    const account = await tx.moneyAccount.findFirst({ where: { id: requestedAccountId, shopId, active: true } });
    if (!account) throw new ApiError(404, 'Money account was not found');
    return account;
  }
  return tx.moneyAccount.findFirst({
    where: { shopId, active: true, type: method },
...
async function applyAccountChange(tx, req, account, amount, direction, note) {
  ...
  await tx.moneyServiceTransaction.create({
    data: {
      ...
      feeMode: 'MANUAL',
      cashChange: account.type === 'CASH' ? direction * amount : 0,
      walletChange: account.type === 'CASH' ? 0 : direction * amount,
  ...
  app.post('/api/business-control/daily-closing', ...close, wrap(async (req, res) => {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.$executeRawUnsafe(
        ...
        req.auth.shopId,
        businessDate,
        Math.max(0, values.todaySaleIncome),
        values.productProfit,
        Math.max(0, values.repairRevenue),
        values.moneyServiceProfit,
        Math.max(0, balances.CASH),
        Math.max(0, balances.KPAY),
        Math.max(0, balances.WAVE_PAY),
        Math.max(0, values.repairIncome),
        values.repairProfit,
        Math.max(0, values.todayExpense),
        Math.max(0, values.otherIncome),
        Math.max(0, values.receivable),
        Math.max(0, values.payable),
        values.todayProfit,
        req.auth.userId,
        note,
      );
